...
function filter(){var q=(search.value||'').toLowerCase().trim();var shown=0;items.forEach(function(d){var hay=(d.getAttribute('data-q')+' '+d.textContent).toLowerCase();var ok=!q||hay.indexOf(q)>=0;d.style.display=ok?'':'none';if(ok)shown++;});if(count)count.textContent=shown+' of '+items.length+' questions shown.';}
if(search)search.addEventListener('input',filter);filter();

var QZ=[{a:0,e:'Antinatalism concerns future births only — never anyone already alive.'},{a:1,e:'It is an ethical argument about creating lives, not hatred or a demand anyone disappear.'}];
