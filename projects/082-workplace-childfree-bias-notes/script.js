...
function render(){var n=boxes.filter(function(b){return b.checked;}).length;if(A.meter)A.meter(bar,n/boxes.length*100);if(count)count.textContent=n+' of '+boxes.length+' noticed.';}
var saved=A.getJSON?A.getJSON('notice',{}):{};saved=saved||{};
boxes.forEach(function(b){var k=b.getAttribute('data-key');if(saved[k])b.checked=true;b.addEventListener('change',function(){saved[k]=b.checked;if(A.setJSON)A.setJSON('notice',saved);render();});});
render();
...
var QZ=[{a:1,e:'The goal is an even distribution that respects everyone — parents included.'},{a:1,e:'Time outside work is everyone\'s own and equally valid.'}];
