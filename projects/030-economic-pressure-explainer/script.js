...
function update(){var items=inputs.map(function(inp){var out=document.getElementById(inp.id.replace('p-','o-'));if(out)out.textContent=WORDS[+inp.value];return {label:inp.getAttribute('data-axis'),value:+inp.value};});if(A.barChart)A.barChart(chart,items,{max:4,fmt:function(v){return WORDS[v];},title:'Economic pressure'});var store={};inputs.forEach(function(inp){store[inp.id]=inp.value;});if(A.setJSON)A.setJSON('pressures',store);}
if(inputs.length){var s=A.getJSON?A.getJSON('pressures',null):null;inputs.forEach(function(inp){if(s&&s[inp.id]!=null)inp.value=s[inp.id];inp.addEventListener('input',update);});update();}

var QZ=[{a:1,e:'They are structural — set by policy and markets — not personal failings.'},{a:1,e:'The fair target is the systems that shape choices, never families or poor people.'}];
