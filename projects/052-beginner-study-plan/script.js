...
try{
var A=window.ANCF||{};
var boxes=[].slice.call(document.querySelectorAll('.plan input[type=checkbox]'));
var bar=document.getElementById('bar'),pct=document.getElementById('pct'),count=document.getElementById('count');
function render(){var n=0;boxes.forEach(function(b){if(b.checked)n++;});if(A.meter)A.meter(bar,n/boxes.length*100);if(pct)pct.textContent=Math.round(n/boxes.length*100)+'%';if(count)count.textContent=n+' of '+boxes.length+' done.';}
if(boxes.length){var s=A.getJSON?A.getJSON('plan',{}):{};s=s||{};boxes.forEach(function(b){var k=b.getAttribute('data-key');b.checked=!!s[k];b.addEventListener('change',function(){s[k]=b.checked;if(A.setJSON)A.setJSON('plan',s);render();});});render();}
var status=document.getElementById('status'),timer=null;
function flash(m){if(!status)return;status.textContent=m;if(timer)clearTimeout(timer);timer=setTimeout(function(){status.textContent='';},1600);}
var resetBtn=document.getElementById('resetBtn');
if(resetBtn)resetBtn.addEventListener('click',function(){if(!window.confirm('Reset your study progress?'))return;boxes.forEach(function(b){b.checked=false;});if(A.remove)A.remove('plan');render();flash('Reset.');});

var QZ=[{a:1,e:'Go at your own pace, for understanding — not speed.'},{a:1,e:'A good plan balances philosophy with the personal and social side.'}];
