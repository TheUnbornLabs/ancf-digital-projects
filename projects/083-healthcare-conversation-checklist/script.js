...
function render(){var n=boxes.filter(function(b){return b.checked;}).length;if(A.meter)A.meter(bar,n/boxes.length*100);if(count)count.textContent=n+' of '+boxes.length+' prepared.';}
var saved=A.getJSON?A.getJSON('prep',{}):{};saved=saved||{};
boxes.forEach(function(b){var k=b.getAttribute('data-key');if(saved[k])b.checked=true;b.addEventListener('change',function(){saved[k]=b.checked;if(A.setJSON)A.setJSON('prep',saved);render();});});
render();
...
var QZ=[{a:1,e:'Repeating back what you heard confirms you understood correctly.'},{a:1,e:'Your qualified provider is the right source for your health questions.'}];
